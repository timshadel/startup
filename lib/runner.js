...
var REQUEST_TIMEOUT = envs.int('REQUEST_TIMEOUT', 45000);
...
server.on('connection', function(socket) {
  ...
  if (!socket.writable) {
    console.log('at=startup-runner warn=timeout writable=' + socket.writable + ' socket=' + socket.id);
  }

  socket.setTimeout(REQUEST_TIMEOUT, function() {
    if (socket.writable && socket.bytesWritten === 0) {
      socket.end('HTTP/1.1 500 Internal Server Error\n\n');
      console.log('at=startup-runner error=timeout status=500-sent writable=' + socket.writable + ' paused=' + socket._paused + ' bytesWritten=' + socket.bytesWritten + ' readyState=' + socket.readyState +' destroyed=' + socket.destroyed + ' socket=' + socket.id);
    } else {
      console.log('at=startup-runner error=timeout status=info writable=' + socket.writable + ' paused=' + socket._paused + ' bytesWritten=' + socket.bytesWritten + ' readyState=' + socket.readyState +' destroyed=' + socket.destroyed + ' socket=' + socket.id);
    }
    setTimeout(function() {
      console.log('at=startup-runner error=timeout status=destroyed socket=' + socket.id);
      socket.destroy();
