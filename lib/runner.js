...
server.on('connection', function(socket) {
  ...
  socket.setTimeout(REQUEST_TIMEOUT, function() {
    if (socket.writable && socket.bytesWritten === 0) {
      ...
      console.log('at=startup-runner error=timeout paused=' + socket._paused + ' status=500-sent socket=' + socket.id);
    } else {
      console.log('at=startup-runner error=timeout status=info writable=' + socket.writable + ' paused=' + socket._paused + ' bytesWritten=' + socket.bytesWritten + ' socket=' + socket.id);
    }
    setTimeout(function() {
